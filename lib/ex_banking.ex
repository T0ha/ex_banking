defmodule ExBanking do
  ...
  def deposit(_user, amount, _currency) when amount <= 0.0, do: {:error, :wrong_arguments}
  ...
  def withdraw(_user, amount, _currency) when amount <= 0.0, do: {:error, :wrong_arguments}
  ...
  @doc """
  """
  @spec send(user(), user(), amount(), currency()) ::
          {:ok, balance(), balance()}
          | {:error,
             :wrong_arguments
             | :not_enough_money
  ...
  def send(_from_user, _to_user, amount, _currency) when amount <= 0.0,
