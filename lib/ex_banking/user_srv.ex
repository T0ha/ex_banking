defmodule ExBanking.UserSrv do
  ...
  def child_spec(user) do
    %{id: user, start: {__MODULE__, :start_link, [user]}}
  end
  ...
  def handle_call({:balance, currency}, _from, %{balances: balances} = state),
    do: {:reply, {:ok, Map.get(balances, currency, 0.0)}, state}

  def handle_call({:withdraw, currency, amount}, _from, %{balances: balances} = state) do
    case Map.get(balances, currency, 0.0) do
      balance when balance >= amount ->
        balances =
          Map.update(balances, currency, amount, fn am -> Float.round(am - amount, 2) end)
